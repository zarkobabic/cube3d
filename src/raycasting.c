...
/*   Updated: 2025/12/06 04:49:25 by eberkau          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cast_ray(double ray_angle, t_point *wall_collision_point, const t_env *env)
{
	...
	directions[ORIENT_EAST] = (ray_angle < M_PI / 2
			|| ray_angle > 3 * M_PI / 2);
	hit.found[WALL_HOR] = check_horizontal_walls(&hit.at[WALL_HOR],
			ray_angle, env, directions[ORIENT_SOUTH]);
	hit.found[WALL_VER] = check_vertical_walls(&hit.at[WALL_VER],
